...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7759945764:AAH0Hv2dz-H1jvXBBWU6eGuNRe_0sBvoy8E'
@RitikXyz099 = "@RitikXyz099"
CO_OWNERS = []  # List of user IDs for co-owners
OWNER_CONTACT = "@RitikXyz099"
ALLOWED_GROUP_IDS = [-1002791268454]
MAX_THREADS = 3600
max_duration = 600
bot_open = False
SPECIAL_MAX_DURATION = 180
...
VPS_FILE = "vps.txt"
BINARY_NAME = "bgmi"
BINARY_PATH = f"/home/master/{bgmi}"
VPS_LIST = []
...
def get_display_name(group_id=None):
    ...
    if group_id is None:
        return GROUP_DISPLAY_NAMES.get('default', f"{@RitikXyz099}")
    return GROUP_DISPLAY_NAMES.get(group_id, GROUP_DISPLAY_NAMES.get('default', f"{@RitikXyz099}"))
...
def load_display_name():
    ...
    if os.path.exists(DISPLAY_NAME_FILE):
        try:
            ...
            GROUP_DISPLAY_NAMES = {'default': f"{@RitikXyz099}"}
    else:
        GROUP_DISPLAY_NAMES = {'default': f"{@RitikXyz099}"}
...
def is_owner(update: Update):
    return update.effective_user.username == @RitikXyz099
...
async def show_users(update: Update, context: CallbackContext):
    ...
    try:
        # Get owner info
        try:
            owner_chat = await context.bot.get_chat(@RitikXyz099)
            owner_info = f"👑 Owner: {owner_chat.full_name} (@{owner_chat.username if owner_chat.username else 'N/A'})"
        except Exception as e:
            owner_info = f"👑 Owner: {@RitikXyz099} (Could not fetch details)"
...
async def add_@RitikXyz099(update: Update, context: CallbackContext):
    """Get owner username and start new bot instance"""
    @RitikXyz099 = update.message.text.strip()
    ...
    process = subprocess.Popen(
        [sys.executable, str(Path(__file__).resolve()), "--token", token, "--owner", @RitikXyz099],
    ...
    display_username = escape_markdown(@RitikXyz099, version=2)
...
async def remove_bot_instance(update: Update, context: CallbackContext):
    ...
    bot_list = "\n".join(
        f"{i}. Owner: @{c['@RitikXyz099']} ({'🟢 Running' if c.get('active') else '🔴 Stopped'})"
...
async def start_bot_selection(update: Update, context: CallbackContext):
    try:
        ...
        if 0 <= selection < len(configs):
            ...
            process = subprocess.Popen(
                [sys.executable, str(Path(__file__).resolve()), "--token", config['token'], "--owner", config['@RitikXyz099']],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE
            )
...
async def generate_key_duration(update: Update, context: CallbackContext):
    ...
    key = f"{@RitikXyz099}-{duration_str}-{unique_key}"
    keys[key] = {
...
def main():
    ...
    global TELEGRAM_BOT_TOKEN, @RitikXyz099
    ...
    if len(sys.argv) > 1 and "--token" in sys.argv:
        ...
        if token_index < len(sys.argv) and owner_index < len(sys.argv):
            ...
            @RitikXyz099 = sys.argv[owner_index]
    ...
    add_bot_handler = ConversationHandler(
        ...
        states={
            ...
            GET_@RitikXyz099: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_@RitikXyz099)],
        },
        fallbacks=[CommandHandler("cancel", cancel_conversation)],
    )
